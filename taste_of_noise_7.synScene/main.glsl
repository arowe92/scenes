...
vec4 renderPassA() {
    ...
    // camera coordinates
    vec2 uv = (fragCoord.xy - RENDERSIZE.xy * 0.5) / RENDERSIZE.y / 4.0;
    vec3 eye = vec3(1,1,1.);
    vec3 at = vec3(0,0,0);
    ...
    // white noise
    vec3 seed = vec3(fragCoord.xy, TIME);
    rng = hash13(seed);
    ...
    tint.xy = _rotate(tint.xy, TIME / 4.0);
    tint.yz = _rotate(tint.yz, TIME / 4.0);
